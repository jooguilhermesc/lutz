...
export default function Citations() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-slate-800">{t('citations.title')}</h2>
      <p className="text-sm text-slate-500">{t('citations.desc')}</p>

      {!running && <ActiveJobPanel jobType="citations" />}

      <div className="card space-y-4">
        <div>
          <label className="label">{t('citations.report.label')}</label>
          <div className="flex gap-2">
            ...
            >
              <option value="">{t('citations.report.select')}</option>
              {reports.map((r) => (
          ...
          {reports.length === 0 && (
            <p className="text-xs text-slate-400 mt-1">{t('citations.report.empty')}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="only-relevant" className="text-sm text-slate-700 cursor-pointer">
            {t('citations.onlyRelevant')}
          </label>
        </div>
        ...
        <div className="flex gap-3 pt-2 items-center flex-wrap">
          <button className="btn-primary" onClick={start} disabled={running || !selectedReport}>
            {running ? t('citations.running') : t('citations.run')}
          </button>
          {running && (
        ...
        {done !== null && (
          <div className={`text-sm font-medium ${done ? 'text-green-600' : 'text-red-600'}`}>
            {done ? t('citations.done') : t('citations.error')}
          </div>
        )}
      </div>
