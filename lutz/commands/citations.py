...
def _citations_user_message(
    ...
    task_line = citation_instructions or _DEFAULT_CITATION_INSTRUCTIONS
    return (
...
@click.option(
    ...
    default=None,
    help=(
        "Custom instructions for the citation extraction LLM call. "
        "Overrides the default task description. Not used with --reading-roadmap."
    ),
)
@click.option(
    "--extract-citations-labels",
    "extract_citations_labels",
    default=None,
    help=(
        "Comma-separated list of relevance codes that trigger citation extraction. "
        "Defaults to 'INCLUDE'. Use when custom verdict categories are configured."
    ),
)
def citations(
    analysis_path: Path,
    ...
    extract_citations_labels: str | None,
) -> None:
    ...
    # ---- classify relevance -------------------------------------------------
    # Prefer the structured `relevance` field written by `lutz analysis --per-article`
    # (INCLUDE / EXCLUDE / UNCERTAIN / UNKNOWN or custom labels).
    # Falls back to regex on the raw analysis text for legacy reports.
    include_labels: tuple[str, ...] = tuple(
        l.strip().upper() for l in (extract_citations_labels or "").split(",")
        if l.strip()
    ) or ("INCLUDE",)

    def _classify(article: dict) -> str:
        structured = (article.get("relevance") or "").upper().strip()
        if not structured or structured == "UNKNOWN":
            return "unknown"
        if structured in include_labels:
            return "relevant"
        if structured == "UNCERTAIN":
            return "unknown"
        if structured:
            return "not_relevant"
        return _parse_relevance(article.get("analysis") or "")

    for a in articles:
        a["_relevance"] = _classify(a)
    ...
    def _process_one(article: dict) -> dict:
        ...
        user_msg = _citations_user_message(prompt_content, filename, analysis_text, context, citation_instructions)
        lang_instr = _LANGUAGE_INSTRUCTIONS.get(language, _LANGUAGE_INSTRUCTIONS["pt"])
