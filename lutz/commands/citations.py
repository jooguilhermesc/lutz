...
def citations(
    ...
    # (INCLUDE / EXCLUDE / UNCERTAIN / UNKNOWN).  Fall back to regex on the raw
    # analysis text for reports generated by older versions.
    def _classify(article: dict) -> str:
        structured = (article.get("relevance") or "").upper().strip()
        if structured == "INCLUDE":
            return "relevant"
        if structured == "EXCLUDE":
            return "not_relevant"
        if structured in ("UNCERTAIN", "UNKNOWN"):
            return "unknown"
